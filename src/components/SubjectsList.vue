      ...
      <v-card class="mx-auto" tile>
        ...
        >
          ...
          <template v-slot:[`item.actions`]="{ item }">
            ...
            <v-icon v-if= "(item.user===currentUser.username)||(currentUser.role === 'admin')" small @click="deleteSubject(item.id)">mdi-delete</v-icon>
          </template>
        </v-data-table>

        <v-card-actions v-if = "(subjects.length > 0)&&(currentUser.role === 'admin')">
